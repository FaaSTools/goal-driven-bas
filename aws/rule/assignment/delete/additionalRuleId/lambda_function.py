...
def delete_assignment(session, conditionRuleId):
    assignment_to_delete = session.query(Assignment).filter(Assignment.conditionRuleId == conditionRuleId).first()
...
def handler(event, context):
    try:
        with SessionLocal() as session:
            conditionRuleId = event.get('conditionRuleId')

            response = delete_assignment(session, conditionRuleId)
            if response:
                return {
                    'statusCode': 200,
                    'headers': headers,
